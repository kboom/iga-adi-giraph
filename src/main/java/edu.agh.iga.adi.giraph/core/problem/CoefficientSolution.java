...
public final class CoefficientSolution implements PartialSolution {
  ...
  public double valueAt(double x, double y) {
    ...
    return b1.getValue(localx) * b1.getValue(localy) * coefficients.get(ielemx, ielemy)
        + b1.getValue(localx) * b2.getValue(localy) * coefficients.get(ielemx, ielemy + 1)
        + b1.getValue(localx) * b3.getValue(localy) * coefficients.get(ielemx, ielemy + 2)
        + b2.getValue(localx) * b1.getValue(localy) * coefficients.get(ielemx + 1, ielemy)
        + b2.getValue(localx) * b2.getValue(localy) * coefficients.get(ielemx + 1, ielemy + 1)
        + b2.getValue(localx) * b3.getValue(localy) * coefficients.get(ielemx + 1, ielemy + 2)
        + b3.getValue(localx) * b1.getValue(localy) * coefficients.get(ielemx + 2, ielemy)
        + b3.getValue(localx) * b2.getValue(localy) * coefficients.get(ielemx + 2, ielemy + 1)
        + b3.getValue(localx) * b3.getValue(localy) * coefficients.get(ielemx + 2, ielemy + 2);
  }
