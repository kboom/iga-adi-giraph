...
TOP_HEADER_COMMAND="$TOP_BASE_CMD | grep -m 1  \"PPID\""
TOP_COMMAND="$TOP_BASE_CMD | grep -Fv -e '%Cpu' -e 'KiB Mem' -e 'KiB Swap' -e 'Threads:' -e 'top -' -e ' PPID ' | grep -v -e '^$' | while IFS= read -r line; do printf '%s %s\n' \"\$(date +%s%3N)\" \"\$line\"; done" # all configuration is expected to be in toprc file
CONVERT_TO_CSV="awk -v suite=\"$RUN\" '{ cmd=substr(\$0,147); gsub(/ +/, \"-\", cmd); print suite,substr(\$0,0,146), cmd}' | sed 's/\( \{1,\}\)/,/g'"

# Top
bash -c "$TOP_HEADER_COMMAND" > "$LOG_TOP_FILE"
