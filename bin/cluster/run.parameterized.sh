...
IGA_CONTAINER_JVM_OPTIONS=${IGA_CONTAINER_JVM_OPTIONS:-"-XX:+PrintFlagsFinal -XX:+UnlockDiagnosticVMOptions -XX:+UseParallelGC -XX:+UseParallelOldGC -XX:+UseNUMA"}
...
exec "${RUN_SCRIPT}" \
  ...
  --config container.java.opts="${IGA_CONTAINER_JVM_OPTIONS}" \
  "${@}"
